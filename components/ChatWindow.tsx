...
export function ChatWindow(props: ChatWindowProps) {
  ...
  const [transactionModalOpen, setTransactionModalOpen] = useState(false); // Initialize state as closed
  const [sourceData, setSourceData] = useState<any[]>([]);
  const [showFullMessage, setShowFullMessage] = useState(false);
  const [trxSuccess, setTrxSuccess] = useState(false);
   

  const openTransactionModal = (sources: any[]) => {
  ...
  return (
   <> <div className={`flex flex-col items-center p-4 md:p-8 mt-16 gap-2 rounded-lg grow overflow-hidden border-none bg-neutral ${(messages.length > 0 ? "border" : "")}`}>
      ...
      <form onSubmit={sendMessage} className="flex w-full flex-col">
        ...
        <div className="flex w-full mt-4">
          <input
            className=" grow mr-8 p-4 border input bg-base-100 rounded-lg"
            value={input}
            placeholder={placeholder ?? "What's it like to be a pirate?"}
            onChange={handleInputChange}
          />
          {trxSuccess ? (
            <button type="submit" className="shrink-0 px-6 py-2 btn rounded w-28">
